...
public class MemTest extends BaseTestCase {
    ...
    public MemTest(Context context, Handler handler, TestCaseInfo testcase) {
    ...
    public boolean onTesting() {
        ...
        int  mem_size = SystemInfoUtils.getMemSize(mContext);
        String str_mem_size = Formatter.formatFileSize(mContext,mem_size*1024);
        final String detal = "\t 内存大小:" + str_mem_size + ",\t   配置大小:" + config_size + "G";
        LogUtil.d(detal);
        config_size = config_size * 1024 * 1024*1024;
        if((Math.abs(mem_size*1024 - config_size)) < 1024*1024*100)
        {
            test_ret = true;
        }
        else
